...
exports.login = async (req, res) => {
  ...
  console.log('  password:     ' + (password ? '(' + String(password).length + ' chars)' : '(empty)'));
  try {
    if (!userId || !password)
    ...
    if (!user) {
      console.log('\n[login] ❌ NO USER FOUND');
      console.log('  Tried these queries on the employees collection:');
      console.log('    { employeeId: "' + raw + '" }');
      console.log('    { employeeId: "' + emailLower + '" }');
      console.log('    { email: /^' + emailLower + '$/i }');
      console.log('    { username: "' + emailLower + '" }');
      console.log('  → no match. The employee row HRMS thinks exists is in a');
      console.log('    different DB, or the email/employeeId is spelled');
      console.log('    differently in the DB than what you typed.\n');
      return res.status(401).json({ message: 'Invalid credentials — no employee found with this email/ID. Check the server logs for the exact lookup queries that ran.' });
    }
    ...
    // Diagnose password issues: if the field is empty/missing, that's
    // a HRMS create-flow bug; if it's there but doesn't match, the
    // operator typed the wrong password.
    const stored = user.password || '';
    if (!stored) {
      console.log('\n[login] ⚠ USER HAS NO PASSWORD ON FILE');
      console.log('  user:', user.email || user.employeeId);
      console.log('  → HRMS must have created this employee without a password.');
      console.log('  → Open HRMS → Edit Employee → set a password → save.\n');
      return res.status(401).json({ message: 'No password set for this employee in the database. Open HRMS, edit the employee, set a password, and save.' });
    }
    const ok = await bcrypt.compare(password, stored);
    if (!ok) {
      console.log('\n[login] ❌ PASSWORD MISMATCH');
      console.log('  user:           ' + (user.email || user.employeeId));
      console.log('  stored hash prefix: ' + stored.slice(0, 10) + '...');
      console.log('  → User exists, but bcrypt.compare returned false.');
      console.log('  → Either you typed the wrong password, OR the password in');
      console.log('    the DB was saved unhashed and now looks like garbage.\n');
...
exports.sendOtp = async (req, res) => {
  ...
  try {
    ...
    // Email failed. In DEV / non-production we ECHO the OTP back in
    // the response so the flow keeps working without a working SendGrid.
    // In PRODUCTION we still 502 so OTPs aren't leaked. Either way we
    // print a giant, easy-to-spot banner in the server logs so the
    // operator can read the OTP from Render's log viewer while
    // SendGrid is being fixed.
    ...
    const hint = /not configured/i.test(reason)
      ...
      : /unauthorized|401|forbidden|403|invalid.*grant|expired|revoked/i.test(reason)
      ? 'SendGrid API key is invalid/revoked. Regenerate it at sendgrid.com → Settings → API Keys.'
    ...
    console.error('');
    console.error('==================================================');
    console.error('  EMAIL UNAVAILABLE — manual OTP fallback');
    console.error('  Email:  ' + normalized);
    ...
    if (process.env.NODE_ENV !== 'production') {
      // DEV fallback — frontend reads `devOtp` and either auto-fills
      // or shows it on the OTP screen so the flow can continue.
      return res.json({
        success:    true,
        emailSent:  false,
        emailError: reason,
        message:    'Email failed — OTP echoed back for dev. Use: ' + otp,
        devOtp:     otp,
        hint,
      });
    }
...
/**
 * GET /api/auth/email-test?to=someone@example.com
 *
 * Sends a real OTP email through whichever provider is configured and
 * returns the result so the operator can verify SendGrid/Resend
 * works without poking at the UI. Returns success info OR the exact
 * provider error message — invaluable for diagnosing "why isn't my
 * OTP arriving" without rebooting the server.
 */
exports.emailTest = async (req, res) => {
  ...
  if (!to || !/^\S+@\S+\.\S+$/.test(to)) {
    return res.status(400).json({
      success: false,
      message: 'Provide a destination email via ?to=user@example.com',
    });
  }
  ...
  return res.json({
    ...
    message: result.sent
      ? `Test email sent via ${result.provider}. Check the inbox of ${to} for the OTP ${testOtp}.`
      : `Email send FAILED. provider=${result.provider}, error=${result.error}`,
  });
};
